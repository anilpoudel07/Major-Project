...
router
  .route("/remove-role/:userId")
  .patch(verifyJWT, requireAdmin, sanitize, validate(roleSchema), removeRole);
router
  .route("/delete-user/:userId")
  .delete(verifyJWT, requireAdmin, sanitize, deleteUser);
router.route("/pending").get(verifyJWT, requireAdmin, getPendingNfcCard);
router
  .route("/verify/:id")
  .patch(verifyJWT, requireAdmin, verifyJWT, verifyNfcCard);
