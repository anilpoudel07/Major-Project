...
export const initiatePayment = async (req, res) => {
  ...
  const txn = await Transcation.findOne({ txnId, status: "payment_required" });
  if (!txn) throw new ApiError(404, "Transaction not found");

  const amountNPR = txn.requiredTopup || txn.fare || 0;
  if (amountNPR < 10) throw new ApiError(400, "Amount must be at least Rs. 10");

  const payload = {
    return_url: process.env.KHALTI_RETURN_URL,
    website_url: process.env.WEBSITE_URL,
    amount: Math.round(amountNPR * 100), // ensure integer paisa
 ...
 try {
  ...
  if (!KhaltiRes?.pidx) {
    throw new ApiError(503, "Khalti returned invalid response");
  }
  const khaltiRes = await initiateKhalti(payload);

  // Safety check - should never be needed if service throws properly
  ...
  txn.khalti = {
    ...
    amount: amountNPR,
    status: "initiated",
    payment_url: khaltiRes.payment_url, // store for later use
  };
...
export const checkPaymentStatus = async (req, res) => {
  ...
  let txn = null;
  if (txnId) {
    txn = await Transcation.findOne({ txnId });
  }
...
export const khaltiCallback = async (req, res) => {
  ...
  if (!pidx) {
    console.error("[KHALTI CALLBACK] Missing pidx");
  ...
  try {
    console.log("[KHALTI CALLBACK] Verifying pidx:", pidx);
    const verificationResult = await verifyKhalti(pidx);

    console.log("[KHALTI CALLBACK] Verification result:", verificationResult);

    // If we reach here → payment is valid
    return res.send(`
      <h2>Payment Successful</h2>
      <p>Status: ${status || 'Completed'}</p>
      <p>Amount processed successfully. Your NFC card balance has been updated.</p>
      <p>You can safely close this window.</p>
    `);
  } catch (err) {
    console.error("[KHALTI CALLBACK] Verification failed:", {
      message: err.message,
      stack: err.stack,
    ...
    const errorMessage = err.message || "Payment verification failed on our side.";
    return res.status(400).send(`
...
export const paymentTranscation = async (req, res) => {
  const { fare } = req.body;
  ...
  const txn = await Transcation.create({
    txnId,
    nfcCard: nfcCard._id,
    passenger: passenger._id,
    fareAmount: fare,          
